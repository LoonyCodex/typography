let make: (~props: option<list<(string, string)>>) => array<Js.t<Template.k>> = (~props) => [
  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundAttachmentMeta.make.name,
    "description": None,
    "key": BackgroundAttachmentMeta.make.name,
    "content": None,
    "args": BackgroundAttachmentMeta.make.args,
    "props": props,
  }),

  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundClipMeta.make.name,
    "description": None,
    "key": BackgroundClipMeta.make.name,
    "content": None,
    "args": BackgroundClipMeta.make.args,
    "props": props,
  }),

  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundColorMeta.make.name,
    "description": None,
    "key": BackgroundColorMeta.make.name,
    "content": None,
    "args": BackgroundColorMeta.make.args,
    "props": props,
  }),

  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundOpacityMeta.make.name,
    "description": None,
    "key": BackgroundOpacityMeta.make.name,
    "content": None,
    "args": BackgroundOpacityMeta.make.args,
    "props": props,
  }),

  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundOriginMeta.make.name,
    "description": None,
    "key": BackgroundOriginMeta.make.name,
    "content": None,
    "args": BackgroundOriginMeta.make.args,
    "props": props,
  }),

  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundPositionMeta.make.name,
    "description": None,
    "key": BackgroundPositionMeta.make.name,
    "content": None,
    "args": BackgroundPositionMeta.make.args,
    "props": props,
  }),

  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundRepeatMeta.make.name,
    "description": None,
    "key": BackgroundRepeatMeta.make.name,
    "content": None,
    "args": BackgroundRepeatMeta.make.args,
    "props": props,
  }),

  Js.Obj.assign(Js.Obj.empty(), {
    "title": BackgroundSizeMeta.make.name,
    "description": None,
    "key": BackgroundSizeMeta.make.name,
    "content": None,
    "args": BackgroundSizeMeta.make.args,
    "props": props,
  }),
]
